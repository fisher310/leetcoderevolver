 ...
 * <p>提示：
 *
 * <p>1 <= candies <= 10^9 1 <= num_people <= 1000
 *
...
class Solution {
  ...
  public int[] distributeCandies(int candies, int num_people) {
    ...
    int remindCandies = candies;
    int currentPeople = 0;
    int currentAllocate = 1;
    while (remindCandies > 0) {
      if (currentAllocate >= remindCandies) {
        ans[currentPeople] += remindCandies;
        break;
      }
      remindCandies -= currentAllocate;
      ans[currentPeople++] += currentAllocate++;
      currentPeople = currentPeople >= num_people ? currentPeople - num_people : currentPeople;
    }
    return ans;
  }
  ...
  public static void main(String[] args) {
    ...
    int[] ans = solution.distributeCandies(7, 4);
